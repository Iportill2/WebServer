...
int	Respons::checkLocation()
{
	...
	for(size_t i = 0; i < server.arLoc.size(); i++)
	{
		if(rq->getUri() == server.arLoc[i]._location)
		{
			...
			std::cout << "url 11111 = " << url << std::endl;
	...
	for(size_t i = 0; i< server.arLoc.size(); i++)
	{
		...
		std::cout << "rq->getUri().substr(0, pos):" <<rq->getUri().substr(0, pos) << "  *  server.arLoc[i]._location:" << server.arLoc[i]._location << std::endl;
		if (rq->getUri().substr(0, pos) == server.arLoc[i]._location)
		{
			_loc = i;
			std::string url = server.arLoc[i]._root.substr(2) + rq->getUri().substr(pos);
			std::cout << "url2222222 = " << url << std::endl;
			if (Utils::isFile(url.c_str()))
				return (_url = url, 1);
			else
				return (0);
	...
	std::string url = server._Root.substr(2) + rq->getUri();
	...
	/* for(size_t i = 0; i < server.arLoc.size(); i++)
	{
		std::cout << "server.arLoc[i]._location:" << server.arLoc[i]._location << std::endl;
		if (server.arLoc[i]._location == "/")
		{
			_loc = i;
			std::string url = server.arLoc[i]._root.substr(2) + rq->getUri();
			std::cout << "url3333333 = " << url << std::endl;
			if (Utils::isFile(url.c_str()))
				return (_url = url, 1);
		}	
	} */
...
{
	std::cout << "rq->getMethod():" << rq->getMethod() << std::endl;
	if (rq->getMethod() == "delete")
	{
		std::cout << "DELETEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE" << std::endl;
	}
	else if (rq->getMethod() == "get")
	{
		...
		else if (!rq->getDownFile().empty() && rq->getUri() == "/deletex")//PROVISIONAL
		{
			if(rq->getUri() == "/delete")
				std::cout << "LOCATON----------------" << rq->getUri() << std::endl;
			Delete res(fd);
